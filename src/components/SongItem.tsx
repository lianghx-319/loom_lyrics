...
export function SongItem({ song, isDark }: SongItemProps) {
  return (
    <a href={`#${song.id}`} className={`items-center grid grid-cols-[max-content_1fr] grid-rows-[max-content_1fr] gap-x-2 gap-y-1 cursor-pointer active:bg-gray-100 ${isDark ? 'bg-gray-800 hover:bg-gray-900/60' : 'bg-white hover:bg-gray-50'} rounded-lg shadow p-2`}>
      <img
